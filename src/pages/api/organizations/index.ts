...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createOrganization() {
    ...
    const body = { ...req.body };
    if (body?.training_material?.length > 0) {
      const create_training_material = body.training_material;
      body.training_material = {
        create: create_training_material,
      };
    } else {
      delete body.training_material;
    }
