...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function getTrainingMaterials() {
    const data = await prisma.training_material
      .withAuthorization({
        roqUserId,
        tenantId: user.tenantId,
        roles: user.roles,
      })
      .findMany(convertQueryToPrismaUtil(req.query, 'training_material'));
  ...
  async function createTrainingMaterial() {
    ...
    const data = await prisma.training_material.create({
      data: body,
    });
    return res.status(200).json(data);
  }
